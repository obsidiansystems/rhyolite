...
module Rhyolite.Backend.Which where
...
staticWhich f = do
  runIO (which f) >>= \case
    ...
    Just f'
      ...
      | otherwise -> compileError $ "Path to executable " <> show f <> " was found in " <> show f' <> " which is not in /nix/store. Be sure to add the relevant package to default.nix."
